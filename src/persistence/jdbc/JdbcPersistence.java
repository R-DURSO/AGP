...
public class JdbcPersistence {
	...
	public Iterator<Transport> getAllMeanOfTransport() {
		...
		try {
			...
			String selectTransportQuery = "SELECT prix," + "confort,vitesse FROM transport";
	...
	public Iterator<NamePosStation> getStation() {
		...
		try {
			...
			String selectNamePosStationQuery = "SELECT DISTINCT nom_station, ST_X(position) as x_coordinate,"
					+ "ST_Y(position) as y_coordinate " + "FROM station";

			PreparedStatement preparedStatement = JdbcConnection.getConnection()
					.prepareStatement(selectNamePosStationQuery);
	...
	public float luxuriousHotelMean() throws SQLException {
		...
		try {
			PreparedStatement preparedStatement = JdbcConnection.getConnection()
					.prepareStatement(selectluxuriousHotelMeanQuery);
			ResultSet result = preparedStatement.executeQuery();
			result.next();
	...
	public float averageHotelMean() throws SQLException {
		...
		try {
			...
			PreparedStatement preparedStatement = JdbcConnection.getConnection()
					.prepareStatement(selectAverageHotelMeanQuery);
	...
	public float ecoHotelMean() throws SQLException {
		...
		try {
			...
			PreparedStatement preparedStatement = JdbcConnection.getConnection()
					.prepareStatement(selectEcoHotelMeanQuery);
	 ...
	 * 
	 * @return an iterator with all the Tourist Attractions who their description
	 *         match the two keyword
	 * @throws ParseException
	 * @throws IOException
	 */
	public Iterator<SiteScore> getTouristAttractionWithKeyWord(String keyWord1, String keyWord2) {
		String SQLQuery = "SELECT id_site, nom_site, type_lieux, niveau_effort, " + "duree_activite, prix,"
		...
		while (siteIteratorSQL.hasNext()) {
			...
			while (scoreIteratorTextual.hasNext()) {
				...
				if (site.getIdSite().equals(id_site)) {
					SiteScore siteScore = new SiteScore(site.getIdSite(), site.getName(), site.getPrice(),
							site.getEffort(), site.getType(), site.getPosition(), site.getDuration(),
							scoreDocName.getScore());
	...
	private Iterator<Site> allTouristAttractions(String selectTouristAttractionsQuery) {
		...
		try {
			...
			PreparedStatement preparedStatement = JdbcConnection.getConnection()
					.prepareStatement(selectTouristAttractionsQuery);
	...
	/**
	 * Public version without param Get all Tourist Attractions
	 * 
	 * @return an iterator with all the TouristAttractions
	 */
